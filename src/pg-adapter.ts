  ...
  implements Queryable
{
  ...
  async executeRaw(query: Query): Promise<Result<number>> {
    ...
    return (await this.performIO(query)).map(
      ({ rowCount: rowsAffected }) => rowsAffected ?? 0
    );
  }
  ...
  private async performIO(
    query: Query,
    catchingUp = false
  ): Promise<Result<pg.QueryArrayResult<any>>> {
    // PATCH: end
  ...
  private async compactPerformIOResult(
    query: Query,
    catchingUp: boolean
    ...
    if (previousQueries) {
      ...
      const previousContextComment = previousQueries.find((q) =>
        isContextComment(q.sql)
      )?.sql;

      if (previousContextComment && isWrite) {
        text = `${sql} ${contextToSqlComment({
          SQL: sql,
          ...sqlCommentToContext(previousContextComment),
        })}`;
      }
      ...
      if (!readyToExecuteTransaction) {
        ...
        if (isBeginQuery(sql) && previousQueries.length === 1)
          return EMPTY_RESULT;

        // Skip accumulated COMMIT
        if (
          isCommitQuery(sql) &&
          previousContextComment &&
          previousQueries.length === 4
        )
          return EMPTY_RESULT;
        ...
        if (
          ...
          for (const prevQuery of previousQueries.slice(
            0,
